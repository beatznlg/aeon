...
export default function Sidebar({
  ...
  useEffect(() => {
    ...
    async function probe() {
      ...
      try {
        ...
        if (!alive) return;
        setSetup(j);
        // Promote health to "warn" only if /api/health is up but a critical key is missing.
        const k = j?.keys || {};
        const nothingWired =
    ...
    probe();
    const t = setInterval(probe, 30_000); // soft poll every 30 s
    return () => {
      alive = false;
      clearInterval(t);
    };
  ...
  const missing =
    setup?.notes?.filter((n) => /missing/i.test(n)) || [];

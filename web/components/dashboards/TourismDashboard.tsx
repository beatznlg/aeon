  ...
  return (
    <motion.section className="module-dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <div className="module-dashboard-header">
        <motion.h2 initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }}>🏨 Hospitality Command Center</motion.h2>
        <motion.p initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.05 }}>Booking optimization, dynamic pricing, and automated concierge.</motion.p>
      </div>
      ...
      <motion.div className="module-kpi-row" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
        ...
        <KPICard title="Positive Sentiment" value={positiveSentiment} sub={`Of ${concierge.length} guests`} color={positiveSentiment > concierge.length / 2 ? "var(--success)" : "var(--warning)"} />
      </motion.div>
            ...
            <div className="space-y-2">
              {concierge.slice(0, 5).map((c, i) => (
                <MiniStatCard
                  ...
                  label={c.intent}
                  value={c.sentiment}
                  color={c.sentiment === "positive" ? COLORS.success : c.sentiment === "negative" ? COLORS.danger : COLORS.warning}
                />
              ))}
            </div>
          </Widget>

...
export function UtilitiesDashboard({ data }: { data: DashboardData }) {
  ...
  const avgRenewable = Math.round(
    grid.reduce((s, g) => s + g.renewable_share_pct, 0) / (grid.length || 1)
  );
  const avgRecycling = Math.round(
    waste.reduce((s, w) => s + w.recycled_pct, 0) / (waste.length || 1)
  );
  ...
  return (
    ...
    >
      <div className="module-dashboard-header">
        ...
        >
          Resource optimization, public service monitoring, waste management, and energy grid
          oversight.
        </motion.p>
      </div>
      ...
      >
        <KPICard
          ...
          sub="Need attention"
          color={criticalResources > 0 ? "var(--danger)" : "var(--success)"}
        />
        <KPICard
          title="Renewable Energy"
          value={`${avgRenewable}%`}
          sub="Grid share"
          color={avgRenewable > 40 ? "var(--success)" : "var(--warning)"}
        />
        <KPICard title="Recycling Rate" value={`${avgRecycling}%`} sub="Average" />
        <KPICard
          title="Grid Critical Zones"
          value={gridCritical}
          sub="Need attention"
          color={gridCritical > 0 ? "var(--danger)" : "var(--success)"}
        />
      </motion.div>
          ...
          <Widget title="Waste Management">
            <FunnelProgress
              stages={[
                {
                  label: "Total Waste (T)",
                  value: Math.round(waste.reduce((s, w) => s + w.total_waste_tons, 0)),
                  max: Math.max(Math.round(waste.reduce((s, w) => s + w.total_waste_tons, 0)), 1),
                  color: "#64748b",
                },
                { label: "Recycled %", value: avgRecycling, max: 100, color: "#10b981" },
                {
                  ...
                  value: Math.round(
                    waste.reduce((s, w) => s + w.collection_efficiency, 0) / (waste.length || 1)
                  ),
                  max: 100,
                  color: "#6366f1",
                },
        ...
        >
          <Widget title="Energy Grid Load">
            ...
            <GaugeChart
              ...
              label="Renewable %"
              max={100}
              color={avgRenewable > 40 ? COLORS.success : COLORS.warning}
            />
          </Widget>
        </motion.div>

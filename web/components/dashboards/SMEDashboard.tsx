  ...
  return (
    <motion.section className="module-dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <div className="module-dashboard-header">
        <motion.h2 initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }}>🏢 SME Business Suite</motion.h2>
        <motion.p initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.05 }}>Workflow automation, document processing, AI support, and supply chain analytics.</motion.p>
      </div>
      ...
      <div className="module-widgets-grid">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.25 }}>
          <Widget title="Document Processing">
            <div className="flex items-center gap-4 mb-3">
              <GaugeChart value={avgConfidence} label="AI Confidence" max={100} color={avgConfidence > 80 ? COLORS.success : avgConfidence > 60 ? COLORS.warning : COLORS.danger} />
            ...
            <div className="space-y-2">
              {tickets.slice(0, 4).map((t, i) => (
                <MiniStatCard
                  ...
                  value={t.sentiment}
                  color={t.escalated ? COLORS.danger : t.sentiment === "positive" ? COLORS.success : COLORS.warning}
                />
              ))}
            </div>
          </Widget>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.35 }}>
          <Widget title="Supply Chain Analytics">
            <FunnelProgress
              stages={[
                { label: "Total Chains", value: chains.length, max: Math.max(chains.length, 1), color: "#6366f1" },
                { label: "Healthy", value: chains.filter((c) => c.risk_level === "low").length, max: chains.length, color: "#10b981" },

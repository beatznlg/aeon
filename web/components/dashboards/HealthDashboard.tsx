...
export function HealthDashboard({ data }: { data: DashboardData }) {
  ...
  const triageData = [
    { name: "Emergent", value: diagnostics.filter((d) => d.urgency === "high").length, color: "#ef4444" },
    { name: "Moderate", value: diagnostics.filter((d) => d.urgency === "moderate").length, color: "#f59e0b" },
    { name: "Low", value: diagnostics.filter((d) => d.urgency === "low").length, color: "#10b981" },
  ].filter((d) => d.value > 0);

  const mockVitalsTrend = vitals.length > 0
  ...
  return (
    <motion.section className="module-dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <div className="module-dashboard-header">
        <motion.h2 initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }}>🏥 Health Command Center</motion.h2>
        <motion.p initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.05 }}>AI diagnostics, patient monitoring, drug interaction checks, and telehealth triage.</motion.p>
      </div>
          ...
          <Widget title="Diagnostic Triage">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 space-y-2">
                {triageData.map((d) => (
                  <MiniStatCard key={d.name} icon={d.name === "Emergent" ? "🔴" : d.name === "Moderate" ? "🟡" : "🟢"} label={d.name} value={d.value} color={d.color} />
                ))}
              </div>
            </div>
          </Widget>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.25 }}>
          <Widget title="Patient Vitals Trend">
            {mockVitalsTrend.length > 0 ? (
              ...
              <div className="text-xs text-aeon-fg-mute text-center py-8">No vitals data available</div>
            )}
          </Widget>
        </motion.div>
            ...
            {interactions.map((d, i) => (
              <div key={i} className="mb-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-semibold text-aeon-fg">{d.medications.join(" + ")}</span>
                  <span className="text-xs text-aeon-fg-mute">({d.interactions_found} interactions)</span>
                </div>
                {d.interactions.length > 0 ? d.interactions.slice(0, 2).map((ix, j) => (
                  <div key={j} className="text-xs text-red-400 bg-red-400/5 rounded p-2 mb-1 border border-red-400/10">
          ...
          <Widget title="Telehealth Triage">
            <div className="space-y-2">
              {telehealth.slice(0, 4).map((t, i) => (
                <div key={i} className="flex items-center justify-between p-2 rounded hover:bg-aeon-bg-2/50">
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                    t.urgency === "emergent" ? "bg-red-400/10 text-red-400" :
                    t.urgency === "urgent" ? "bg-amber-400/10 text-amber-400" :

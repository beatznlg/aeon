  ...
  return (
    <motion.section className="module-dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <div className="module-dashboard-header">
        <motion.h2 initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }}>🎭 Cultural Heritage Command Center</motion.h2>
        <motion.p initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.05 }}>Visitor engagement, heritage site insights, exhibition planning, and virtual tours.</motion.p>
      </div>
      ...
      <motion.div className="module-kpi-row" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
        ...
        <KPICard title="Engagement Score" value={`${avgEngagement}%`} sub="Average" color={avgEngagement > 65 ? "var(--success)" : "var(--warning)"} />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }}>
          <Widget title="Visitor Engagement">
            <div className="flex items-center gap-4 mb-3">
              <GaugeChart value={avgEngagement} label="Engagement" max={100} color={avgEngagement > 65 ? COLORS.success : avgEngagement > 40 ? COLORS.warning : COLORS.danger} />
              <div className="flex-1 space-y-2">
                {visitors.slice(0, 3).map((v, i) => (
                  <MiniStatCard key={i} icon="👥" label={v.venue} value={`${v.daily_visitors}/day`} />
                ))}
              </div>
            </div>
          </Widget>

...
export function ManufacturingDashboard({ data }: { data: DashboardData }) {
  ...
  const avgTemp = Math.round(
    maintenance.reduce((s, m) => s + m.temp_c, 0) / (maintenance.length || 1)
  );

  const machineMetrics = [
    {
      name: "Healthy",
      value: maintenance.filter((m) => m.status === "healthy").length,
      color: "#10b981",
    },
    {
      name: "Warning",
      value: maintenance.filter((m) => m.status === "warning").length,
      color: "#f59e0b",
    },
    {
      name: "Critical",
      value: maintenance.filter((m) => m.status === "critical").length,
      color: "#ef4444",
    },
  ].filter((d) => d.value > 0);
  ...
  return (
    ...
    >
      <div className="module-dashboard-header">
        ...
        >
          Predictive maintenance, quality control, and smart logistics monitoring.
        </motion.p>
      </div>
      ...
      >
        ...
        <KPICard
          ...
          value={`${((qc.reduce((s, q) => s + q.defect_rate, 0) / (qc.length || 1)) * 100).toFixed(2)}%`}
          sub="Across batches"
        />
        <KPICard
          ...
          sub="Machine average"
          color={
            avgTemp > 80 ? "var(--danger)" : avgTemp > 60 ? "var(--warning)" : "var(--success)"
          }
        />
        <KPICard
          ...
          value={delayedRoutes}
          sub="Routes delayed"
          color={delayedRoutes > 0 ? "var(--danger)" : "var(--success)"}
        />
      </motion.div>
          ...
          <Widget title="Machine Health Overview">
            <div className="grid grid-cols-2 gap-3 mb-3">
              <GaugeChart
                ...
                max={120}
                color={
                  avgTemp > 80 ? COLORS.danger : avgTemp > 60 ? COLORS.warning : COLORS.success
                }
              />
              <GaugeChart
                value={atRisk > 0 ? Math.min(atRisk * 25, 100) : 5}
                label="Risk Index"
                max={100}

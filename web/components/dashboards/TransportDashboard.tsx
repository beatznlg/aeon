...
export function TransportDashboard({ data }: { data: DashboardData }) {
  ...
  const avgUtilization = Math.round(
    fleet.reduce((s, f) => s + f.utilization_pct, 0) / (fleet.length || 1)
  );
  const avgCongestion = Math.round(
    traffic.reduce((s, t) => s + t.current_congestion, 0) / (traffic.length || 1)
  );
  ...
  return (
    ...
    >
      <div className="module-dashboard-header">
        ...
        >
          Traffic optimization, fleet scheduling, and route planning.
        </motion.p>
      </div>
      ...
      >
        ...
        <KPICard
          ...
          sub="Nearby"
          color={activeIncidents > 0 ? "var(--danger)" : "var(--success)"}
        />
          ...
          <Widget title="Traffic Congestion">
            ...
            <div className="mt-3 space-y-2">
              {traffic.slice(0, 3).map((t, i) => (
                <MiniStatCard
                  key={i}
                  icon={t.incident_nearby ? "🚨" : "🚦"}
                  label={t.zone}
                  value={`${t.current_congestion}/10`}
                  color={
                    t.current_congestion > 6
                      ? COLORS.danger
                      : t.current_congestion > 3
                        ? COLORS.warning
                        : COLORS.success
                  }
                />
        ...
        >
          <Widget title="Fleet Utilization">
            ...
            <div className="mt-2 text-xs text-center text-aeon-fg-mute">
              {fleet.length > 0 &&
                `${fleet.reduce((s, f) => s + f.vehicles_available, 0)} vehicles across ${fleet.length} fleets`}
            </div>
          </Widget>
        </motion.div>

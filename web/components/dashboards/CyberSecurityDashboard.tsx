...
export function CyberSecurityDashboard({ data }: { data: DashboardData }) {
  ...
  const activeThreats = threats.filter(
    (t) => t.status === "blocked" || t.status === "quarantined"
  ).length;
  ...
  const severityData = [
    ...
    {
      name: "Medium",
      value: vulns.filter((v) => v.severity.toLowerCase() === "medium").length,
      color: "#06b6d4",
    },
    {
      name: "Low",
      value: vulns.filter((v) => v.severity.toLowerCase() === "low").length,
      color: "#10b981",
    },
  ].filter((d) => d.value > 0);
  ...
  const timelineEvents = threats.slice(0, 5).map((t) => ({
    ...
    type: (t.severity === "critical" ? "danger" : t.severity === "high" ? "warn" : "ok") as
      "ok" | "warn" | "danger",
  }));
  ...
  return (
    ...
    >
      <div className="module-dashboard-header">
        ...
        >
          Threat intelligence, vulnerability tracking, IP reputation, and compliance posture.
        </motion.p>
      </div>
      ...
      >
        <KPICard
          ...
          value={activeThreats}
          sub="Blocked / quarantined"
          color={activeThreats > 0 ? "var(--danger)" : "var(--success)"}
        />
        <KPICard
          title="Critical Vulns"
          value={criticalVulns}
          sub="Need immediate patch"
          color={criticalVulns > 0 ? "var(--danger)" : "var(--success)"}
        />
        {ip && (
          <KPICard
            title="IP Rep Score"
            value={ip.score.toFixed(2)}
            sub="0-1 reputation"
            color={ip.score > 0.5 ? "var(--danger)" : "var(--success)"}
          />
        )}
        {compliance && (
          <KPICard title="Compliance" value={`${compliance.score}%`} sub={compliance.framework} />
        )}
      </motion.div>

      <div
        className="module-widgets-grid"
              ...
              <div className="mt-3 space-y-1">
                {threats.slice(0, 3).map((t, i) => (
                  <MiniStatCard
                    ...
                    label={t.type}
                    value={t.indicator}
                    color={t.severity === "critical" ? COLORS.danger : COLORS.warning}
                  />
                ))}
              </div>
            )}
          ...
          <Widget title="Vulnerability Posture">
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 text-center">
              ...
              <MiniStatCard
                icon="🟢"
                label="Patched"
                value={vulns.filter((v) => v.patch_available).length}
          ...
          >
            <Widget title="Compliance Posture">
              <div className="module-elasticity">
                ...
                <div>
                  <span>Maturity</span>
                  <strong>{compliance.maturity}</strong>
                </div>
              </div>
          ...
          >
            <Widget title="IP Reputation">
              <div className="flex items-center gap-4">
                <GaugeChart
                  value={Math.round((1 - ip.score) * 100)}
                  label="Reputation"
                  max={100}
                  color={ip.score > 0.5 ? COLORS.danger : COLORS.success}
                />
                <div className="space-y-1 text-xs text-aeon-fg-mute">
                  <div>
                    Known malicious:{" "}
                    <strong className="text-aeon-fg">{ip.known_malicious ? "Yes" : "No"}</strong>
                  </div>
                  <div>
                    Sources:{" "}
                    <strong className="text-aeon-fg">{ip.source_countries.join(", ")}</strong>
                  </div>
                  <div>
                    Last seen: <strong className="text-aeon-fg">{ip.last_seen_days}d ago</strong>
                  </div>
                </div>
              </div>
            </Widget>
              ...
              <div className="space-y-2">
                {data.security_news.slice(0, 4).map((n, i) => (
                  <a
                    key={i}
                    href={n.url}
                    className="block text-xs text-aeon-fg hover:text-aeon-primary transition-colors p-2 rounded hover:bg-aeon-bg-2/50"
                  >
                    📰 {n.title}
                  </a>
                ))}

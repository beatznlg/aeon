...
function mockTick(appId: string, query: string): object {
  ...
  if (appId === "cybersecurity") {
    if (lower.includes("scan") || lower.includes("vuln")) {
      return {
        ok: true, app_id: "cybersecurity", query,
        scan_results: [
  ...
  if (appId === "retail") {
    if (lower.includes("forecast") || lower.includes("demand")) {
      return { ok: true, app_id: "retail", query, forecast: { sku: "SKU-001", predicted_units: 1240, confidence: 0.92, trend: "rising" } };
    }
    if (lower.includes("inventory") || lower.includes("stock")) {
      return { ok: true, app_id: "retail", query, inventory_health: { total_stock: 5250, low_stock_count: 1, overstock_count: 1 } };
    }
    return fallback;
  }
  ...
  if (appId === "manufacturing") {
    if (lower.includes("maintenance")) {
      return { ok: true, app_id: "manufacturing", query, maintenance_schedule: [
  ...
  if (appId === "professional") {
    if (lower.includes("document") || lower.includes("legal")) {
      return { ok: true, app_id: "professional", query, document_analysis: { clauses_found: 14, risk_flags: ["indemnification cap"], summary: "Standard NDA reviewed" } };
    }
    return fallback;
  }
  ...
  if (appId === "tourism") {
    if (lower.includes("pricing") || lower.includes("revenue")) {
      return { ok: true, app_id: "tourism", query, pricing_analysis: { room: "King Suite", current: 299, optimal: 320, recommendation: "increase by 7%" } };
    }
    return fallback;
  }
  ...
  if (appId === "health") {
    if (lower.includes("vitals") || lower.includes("monitor")) {
      return { ok: true, app_id: "health", query, vitals_summary: { patients_monitored: 42, alerts: 3, stable: 39 } };
    }
    if (lower.includes("drug") || lower.includes("interaction")) {
      return { ok: true, app_id: "health", query, interaction_check: { medications: ["aspirin", "warfarin"], risk: "moderate", warning: "increased bleeding risk" } };
    }
    return fallback;
  }
  ...
  if (appId === "transport") {
    if (lower.includes("route") || lower.includes("optimize")) {
      return { ok: true, app_id: "transport", query, route_optimization: { stops: ["A", "B", "C"], estimated_distance_km: 42, estimated_time_min: 55, fuel_cost: 18.5 } };
    }
    if (lower.includes("traffic") || lower.includes("congestion")) {
      return { ok: true, app_id: "transport", query, traffic_summary: { zones_monitored: 4, congested_zones: 2, avg_speed_kmh: 25 } };
    }
    return fallback;
  }
  ...
  if (appId === "finance") {
    if (lower.includes("risk") || lower.includes("var")) {
      return { ok: true, app_id: "finance", query, risk_assessment: { portfolio_value: 500000, var_95_1d: 12500, sharpe: 1.45, recommendation: "diversify fixed income" } };
    }
    if (lower.includes("fraud")) {
      return { ok: true, app_id: "finance", query, fraud_summary: { flagged_transactions: 3, total_blocked: 1, review_queue: 2 } };
    }
    return fallback;
  }
  ...
  if (appId === "cultural_heritage") {
    if (lower.includes("visitor") || lower.includes("engagement")) {
      return { ok: true, app_id: "cultural_heritage", query, visitor_engagement: { venues: 3, avg_engagement: 88, top_strategy: "extend hours weekends" } };
    }
    if (lower.includes("exhibition") || lower.includes("plan")) {
      return { ok: true, app_id: "cultural_heritage", query, exhibition_plan: { theme: "Modern Art", projected_visitors: 80000, roi: 2.4 } };
    }
    return fallback;
  }
  ...
  if (appId === "utilities") {
    if (lower.includes("grid") || lower.includes("energy")) {
      return { ok: true, app_id: "utilities", query, grid_status: { regions: 3, stable: 2, critical: 1, avg_utilization: 89 } };
    }
    if (lower.includes("waste")) {
      return { ok: true, app_id: "utilities", query, waste_summary: { districts: 3, avg_recycling_pct: 32.7, collection_efficiency: 0.88 } };
    }
    return fallback;
  }
  ...
  if (appId === "sme") {
    if (lower.includes("workflow") || lower.includes("automation")) {
      return { ok: true, app_id: "sme", query, workflow_summary: { processes_automated: 3, hours_saved_monthly: 260, annual_savings: 52000 } };
    }
    if (lower.includes("support") || lower.includes("ticket")) {
      return { ok: true, app_id: "sme", query, support_summary: { tickets_resolved: 142, escalated: 3, avg_satisfaction: 4.3 } };
    }
    return fallback;
  }

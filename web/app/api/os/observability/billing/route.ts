...
export async function GET(req: Request) {
  ...
  if (!url) {
    return NextResponse.json({
      ...
      billing: {
        ...
        plan: { id: "free", name: "Free", limits: { requests: 1000, tokens: 100000, workflows: 10, integrations: 5 } },
        credits: 0,
  ...
  try {
    const res = await fetch(`${url}/billing/${encodeURIComponent(workspaceId)}?days=${days}`, { cache: "no-store" });

...
function LoginForm() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError("Unable to reach AEON. Check the connection and try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const demo = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.email || !data.password) {
        setError(data.error || "Demo account is not ready yet.");
        return;
      }
      const ok = await finishSignIn(data.email, data.password);
      if (ok) router.push("/onboarding?demo=1");
    } catch {
      setError("Demo service is unavailable. Please try again in a moment.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <form className="auth-form" onSubmit={submit} noValidate>
          ...
          <label>Password
            <span className="auth-password">
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" autoComplete={mode === "login" ? "current-password" : "new-password"} minLength={6} required />
              <button type="button" className="auth-show" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? "Hide" : "Show"}</button>
            </span>
          </label>

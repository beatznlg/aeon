# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  AEON α — single-cell version                                             ║
# ║  Paste this whole block as ONE cell in a new Colab notebook.              ║
# ║  Runtime → T4 GPU. Secrets (left tab):                                    ║
# ║    TELEGRAM_BOT_TOKEN  (required)                                          ║
# ║    GROQ_API_KEY        (recommended — parasite fallback, free tier)        ║
# ║    GEMINI_API_KEY      (recommended — vision + audio, free tier)           ║
# ║    HF_API_TOKEN        (optional — musicgen)                              ║
# ║    WEB3_PRIVATE_KEY    (optional — earning on Base L2)                   ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
Z = zstd if False else None  # placeholders
try:
...
# ─── 6. Skill DAG (Drive-mirror, hot+cold RAM tier) ───────────────────────────
class SkillDAG:
...
# ─── 7. Subprocess sandbox (no Docker on free Colab) ──────────────────────────
def sandbox_run(source: str, timeout: int = 6, mem_mb: int = 128):
...
async def verify_payment(tx_hash: str, expected_to: str, min_usdc6: int) -> Dict[str, Any]:
    ...
    try:
        ...
        for log in receipt["logs"]:
            if log["address"].lower() != USDC_BASE.lower(): continue
            if log["topics"][0] != USDC_TRANSFER_TOPIC: continue
            to_addr = "0x" + (log["topics"][2].hex().lstrip("0x").rjust(40, "0"))[-40:]
...
# ─── 10. Multil-modal: Gemini (vision + audio understanding), edge-tts ──────
async def gemini_text(prompt: str, image_path: Optional[Path] = None,
    ...
    try:
        ...
        if audio_path:
            parts.append({"mime_type": "audio/ogg",
...
async def gemini_video_summary(video_path: Path, prompt="describe briefly",
                                max_frames=6) -> str:
    try:
        import cv2
...
# ─── 12. The single `ask()` — chat, code, vision, audio ──────────────────────
...
if TELEGRAM:
    ...
    def _audio(m):
        """Same handling as voice (audio files sent as documents)."""
        try:
...
# ─── 14. Boot: write diary + start AEON loop + start polling ─────────────────
def _session_diary():
    p = AEON_ROOT / "diary"
...
if TELEGRAM:
    # Start AEON's event loop in a background thread
    def _loop_runner():
    ...
    # exited polling — usually means token revoked
    _safe_reply_local = lambda x: log.info(f"polling ended: {x}")
else:
    log.info("TELEGRAM_BOT_TOKEN not set. AEON is headless.")
    log.info("Add the token in Colab Secrets and re-run this cell.")
